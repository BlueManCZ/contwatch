...
class FlaskWebServer:
    def __init__(self, _manager: HandlerManager, _database):
        ...
        def page(page_name):
            return render_template(
                "layouts/default.html", manager=self.manager, page=page_name
            )
        ...
        def overview():
            ...
            messages.reverse()
            return render_template(
                "pages/overview.html", manager=self.manager, messages=messages
            )
        ...
        def inspector():
            return render_template(
                ...
                views=self.database.get_chart_views(),
            )
                    ...
                    for message in self.manager.message_queue:
                        if message["type"] == "event" and not message["incoming"]:
                            if message["queue_index"] == int(request.json[attribute]):
                                ...
                                break
            return render_template(
                "pages/actions.html", manager=self.manager, routine_log=routine_log
            )

        @self.app.route("/handlers", methods=["POST"])
        def handlers():
            return render_template(
                "pages/handlers.html", handlers=self.manager.get_handlers()
            )
        ...
        def data():
            ...
            for handler_id in self.manager.registered_handlers:
                storage_attributes = self.manager.registered_handlers[
                    handler_id
        ...
        def build_chart_data(data_map, datetime_from, datetime_to, smartround):
            ...
            for handler_id in data_map:
                ...
                for attribute in attributes:
                    ...
                    result = self.database.get_handler_attribute_data(
                        handler_id,
                        attribute,
                        datetime_from,
                        datetime_to,
                    ...
                    for entry in result:
                        entry_time = entry[0]
                        chart_data[attribute]["timestamps"].append(
        ...
        def build_chart_event_data(
            handler_id, event, event_type, datetime_from, datetime_to
        ):
            response = {}
            ...
            result = self.database.get_handler_stored_event_data(
                handler_id,
                event,
                True if event_type == "in" else False,
                datetime_from,
                datetime_to,
            )
            for entry in result:
        ...
        def api_charts():
            handler_id = (
                request.args["handler_id"] if "handler_id" in request.args else ""
            )
            ...
            smartround = (
                request.args["smartround"] if "smartround" in request.args else 0
            )
            ...
            if handler_id:
                try:
                    ...
                    print(e)
                    return tools.json_error(
                        400, "Argument 'handler_id' has to be an integer."
                    )
            elif raw_query:
                try:
                    ...
                    print(e)
                    return tools.json_error(
                        400, "Argument 'query' cannot be successfully parsed"
                    )
            else:
                return tools.json_error(
                    400,
                    "Either 'handler_id' or 'query' has to be provided as an argument.",
                )
            ...
            if date_from:
                try:
                    ...
                    print(e)
                    return tools.json_error(
                        400, "Argument 'date_from' is not in '%Y-%m-%d' format."
                    )
            ...
            if date_to:
                try:
                    ...
                    print(e)
                    return tools.json_error(
                        400, "Argument 'date_to' is not in '%Y-%m-%d' format."
                    )
            ...
            if smartround:
                try:
                    ...
                    print(e)
                    return tools.json_error(
                        400, "Argument 'smartround' has to be an integer."
                    )
        ...
        def api_events():
            ...
            if handler_id:
                ...
                return tools.json_error(
                    400, "Argument 'handler_id' has to be provided as an argument."
                )
            ...
            return build_chart_event_data(
                handler_id, event_name, event_type, datetime_from, datetime_to
            )
        ...
        def api_data():
            ...
            return self.manager.last_messages[handler_id][1]
        ...
        def dialog(dialog_name):
            ...
            if "add_new_routine" == dialog_name:
                ...
                target_workflow = self.manager.event_manager.get_workflow(
                    int(request.json["target_workflow"])
                )
        ...
        @self.app.route(
            "/edit_json_attributes_to_store/<int:handler_id>", methods=["POST"]
        )
        ...
        def save_chart_view():
            ...
            if view_id >= 0:
                self.database.update_chart_view(
                    view_id, request.json["label"], request.json["settings"]
                )
            else:
                view_id = self.database.add_chart_view(
                    request.json["label"], request.json["settings"]
                ).id
        ...
        def add_new_event_listener():
            ...
            if not label:
                return tools.json_notif(
                    400, "error", "Empty name", f"Event name cannot be empty."
                )
            ...
            if self.manager.event_manager.add_event_listener(listener):
                ...
                if workflow_id:
                    ...
                    listener.set_workflow(
                        self.manager.event_manager.get_workflow(workflow_id)
                    )
                data_listener_status = (
                    bool(request.form["data_listener"])
                    if "data_listener" in request.form
                    else False
                )
        ...
        def delete_workflow(workflow_id):
            ...
            for workflow_instance in self.manager.event_manager.get_workflows():
                for routine in workflow_instance.routines.copy():
                    if (
                        routine.type == "perform_workflow"
                        and routine.get_config()["workflow"] == workflow_id
                    ):
                        self.manager.event_manager.delete_routine(routine.id)
        ...
        def hr_datetime(seconds):
            ...
            for name, value in time_data.items():
                if value > 0:
                    time_parts.append(
                        f"{round(value)} {name}{'s' if value > 1 else ''}"
                    )
            ...
            while self.active:
                ...
                for query in self.cache:
                    for entry in self.cache[query]:
                        if (
                            entry["t"].day == now.day
                            and entry["t"].month == now.month
                            and entry["t"].year == now.year
                        ):
                            if settings.WEB_SERVER_DEBUG:
                                print("Rebuilding cache")
                            entry["r"] = build_chart_data(
                                parse_query(query), entry["f"], now, entry["s"]
                            )
                            if settings.WEB_SERVER_DEBUG:
    ...
    def run(self):
        self.sio.run(
            ...
            self.host,
            self.port,
            debug=settings.WEB_SERVER_DEBUG,
            use_reloader=False,
        )
